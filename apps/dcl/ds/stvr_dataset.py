...
import pdb
# LMSV存储图片文件
from apps.dcl.ds.image_lmdb import ImageLmdb

class StvrDataset(data.Dataset):
    # 数据集图片文件保存方法
    IMGM_FILE = 1
    IMGM_LMDB = 2
    ...
    def __init__(self, Config, anno, swap_size=[7,7], common_aug=None, swap=None, totensor=None, train=False, train_val=False, test=False):
        self.img_mode = StvrDataset.IMGM_FILE
        if StvrDataset.IMGM_LMDB == self.img_mode:
            print('使用lmdb保存图片...')
        ...
        if isinstance(anno, pd.core.frame.DataFrame):
            ...
            StvrDataset.shuffle_samples(self.paths, self.brand_labels, self.bmy_labels)
    ...
    def pil_loader(self,imgpath):
        if StvrDataset.IMGM_FILE == self.img_mode:
    ...
    def preprocess_anno():
        ...
        paths ,brand_labels, bmy_labels = None, None, None
        if isinstance(anno, pd.core.frame.DataFrame):
            ...
            bmy_labels = anno['bmy_label'].tolist()
        StvrDataset.shuffle_samples(paths, brand_labels, bmy_labels)
        paths_len = len(paths)
